...
class Statusbar:
    ...
    @classmethod
    def blit(cls, level: Level, screen: pygame.Surface | None = None, rescaling: bool = True):
        """Blits the rendered status bar on the top of the screen.
        If rescaling is True, the status bar will be stretched to the user's screen width"""
        ...
        cls.missiles = Layout.pad_surface(cls.font.render(f"{level.ship.missiles%100:02d}", False, COLOR.WHITE), cls.stats_padding, vertical_padding=True, horizontal_padding=True)
        cls.level_number = Layout.pad_surface(cls.font.render(f"L{level.number%100:02d}", False, COLOR.WHITE), cls.stats_padding, vertical_padding=True, horizontal_padding=True)
        cls.lives = Layout.pad_surface(cls.font.render(f"{level.ship.lives%100:02d}", False, COLOR.WHITE), cls.stats_padding, vertical_padding=True, horizontal_padding=True)
        cls.first_row = Layout.align_surfaces([cls.level_number,cls.lives_icon,cls.lives,
        ...
        cls.both_rows = Layout.align_surfaces([cls.first_row,cls.second_row], "vertical", spacing=cls.h//32)
        if rescaling:
